...
def main(args):
    with ipdb.launch_ipdb_on_exception():
        ...
        if args.att_type == 'ind':
            train_file = os.path.join(CFG["gpudatadir"], f"ft_classif_supervised_ind{suffix}.train")
            test_file = os.path.join(CFG["gpudatadir"], f"ft_classif_supervised_ind{suffix}.test")
        elif args.att_type == 'exp':
            train_file = os.path.join(CFG["gpudatadir"], f"ft_classif_supervised_exp_5{suffix}.train")
            test_file = os.path.join(CFG["gpudatadir"], f"ft_classif_supervised_exp_5{suffix}.test")
        elif args.att_type == 'delta':
            train_file = os.path.join(CFG["gpudatadir"], f"ft_classif_supervised_delta_5{suffix}.train")
            test_file = os.path.join(CFG["gpudatadir"], f"ft_classif_supervised_delta_5{suffix}.test")
        elif args.att_type == 'ind_user':
            ...
            raise Exception("Wrong att type specified! Can be \"ind\", \"exp\" or \"delta\" got: " + args.att_type)
                ...
                for epochs in [5, 25, 50]:
                    ...
                    for word_ngram in [1, 3, 5]:
                        ...
                        try:
                            ...
                            continue
                        pbar.update(1)
                with open(os.path.join(CFG["gpudatadir"], f"gs_ft_classif{args.att_type}.pkl"), "wb") as f:
                    pkl.dump(results, f)
        if args.TEST == "True":
            with open(os.path.join(CFG["gpudatadir"], f"gs_ft_classif{args.att_type}.pkl"), "rb") as f:
                results = pkl.load(f)
            best_f1_test = 0
            best_f1_train = 0
        ...
        # exp best test : 0.31094171390830533 for (0.1, 5, 3)
        # ind best test :  27.350844845616155 for With params (0.1, 25, 1)
