...
from models.classes.RewriterDelta import RewriterDelta
...
def main(hparams):
    ...
    if hparams.TRAIN == "True":
        ...
        train_loader = DataLoader(dataset_train, batch_size=hparams.b_size, collate_fn=collate_for_rewriter_light,
                                  num_workers=hparams.num_workers, shuffle=True, drop_last=True, pin_memory=True)
        valid_loader = DataLoader(dataset_valid, batch_size=hparams.b_size, collate_fn=collate_for_rewriter_light,
    ...
    if hparams.TEST == "True":
        ...
        test_loader = DataLoader(dataset_test, batch_size=1, collate_fn=collate_for_rewriter_light,
...
def load_datasets(hparams, splits, load):
    datasets = []
    common_hparams = {'data_dir': CFG["gpudatadir"],
                     "max_len": hparams.max_len,
                     "subsample": hparams.subsample,
                     "is_toy": hparams.toy_dataset == "True",
                     "load": load}
    for split in splits:
        datasets.append(DeltaIndJobsCustomVocabDataset(**common_hparams, split=split))
    return datasets
...
if __name__ == "__main__":
    ...
    # model attributes
    parser.add_argument("--b_size", type=int, default=90)
    ...
    parser.add_argument("--model_type", type=str, default="realDelta_rdmTidle_title")
