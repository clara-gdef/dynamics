...
class CamembertRewriter(pl.LightningModule):
    def __init__(self, datadir, emb_size, desc, vocab_size, alpha, beta, model_path, hp):
        ...
        self.decoder = CamembertForCausalLM.from_pretrained('camembert-base')
        #self.decoder.set_output_embeddings(self.encoder.embeddings.word_embeddings)
    ...
    def forward(self, jobs, delta_indices, ind_indices, delta_tilde_indices, ind_tilde_indices, fwd_type, batch_nb):
        if fwd_type == "DAE":
            ...
            inputs = self.tokenizer(jobs, truncation=True, padding="max_length", max_length=self.max_len,
                                    return_tensors="pt")
            input_tokenized, mask = inputs["input_ids"].to(self.decoder.device), inputs["attention_mask"].to(
                self.decoder.device)
            max_seq_len = input_tokenized.shape[-1]
            inputs_corrupted = self.add_noise(input_tokenized,
                                              mask,
                                              max_seq_len)
            encoder_outputs = self.encoder(input_ids=inputs_corrupted, attention_mask=mask)['last_hidden_state']
            inputs_corrupted_embedded = self.encoder.embeddings(inputs_corrupted)
            inputs_corrupted_embedded[:, 0, :] = attributes_as_first_token[:, 0, :]

            decoder_out = self.decoder(inputs_embeds=inputs_corrupted_embedded[:, :-1],
                                       attention_mask=mask[:, :-1].to(self.decoder.device).long(),
                                       encoder_hidden_states=encoder_outputs.to(self.decoder.device).float(),
                                       labels=input_tokenized[:, 1:].to(self.decoder.device).long())
            biased_outputs = decoder_out.logits + attributes_as_bias
            loss = torch.nn.functional.cross_entropy(biased_outputs.transpose(-1, 1),
            ...
            input_tokenized, mask = inputs["input_ids"].to(self.decoder.device), inputs["attention_mask"].to(
                self.decoder.device)
            max_seq_len = input_tokenized.shape[-1]
            encoder_outputs = self.encoder(input_ids=input_tokenized, attention_mask=mask)['last_hidden_state']

            previous_token = atts_tilde_as_first_token
            decoded_tokens, decoder_outputs = [], []

            for di in range(max_seq_len - 1):
                decoder_output = self.decoder(
                    inputs_embeds=previous_token,
                    attention_mask=mask[:, di].unsqueeze(1).to(self.decoder.device).long(),
                    encoder_hidden_states=encoder_outputs.to(self.decoder.device).float())
                decoder_output_biased = decoder_output.logits #+ atts_tilde_as_bias
                decoder_outputs.append(decoder_output_biased)
                decoder_tok = torch.argmax(decoder_output_biased[:, di], dim=-1)
                decoded_tokens.append(decoder_tok)
                previous_token = torch.cat((previous_token, self.encoder.embeddings(decoder_tok.unsqueeze(-1))), dim=1)
            #ipdb.set_trace()
            ...
            inputs2 = self.tokenizer(decoded_jobs, truncation=True, padding="max_length", max_length=self.max_len,
                                     return_tensors="pt")
            decoded_x_tilde, new_mask = inputs2["input_ids"].to(self.decoder.device), inputs2["attention_mask"].to(
                self.decoder.device)
            encoder_outputs_2 = self.encoder(decoded_x_tilde, new_mask)["last_hidden_state"]
            embedded_decoded_jobs = self.encoder.embeddings(decoded_x_tilde)
            embedded_decoded_jobs[:, 0, :] = atts_as_first_token[:, 0, :]

            decoder_out = self.decoder(inputs_embeds=embedded_decoded_jobs[:, :-1],
                                       attention_mask=mask[:, :-1].to(self.decoder.device).long(),
                                       encoder_hidden_states=encoder_outputs_2.to(self.decoder.device).float(),
                                       labels=input_tokenized[:, 1:].to(self.decoder.device).long()
                                       )
            biased_outputs = decoder_out.logits + atts_as_bias
            loss = torch.nn.functional.cross_entropy(biased_outputs.transpose(-1, 1),
                                                     input_tokenized[:, 1:], reduction="sum", ignore_index=1)
            # loss = loss_gen + loss_classif*sum(sum(new_mask)) + loss_ppl
            if self.hp.print_preds == "True" and batch_nb % 1 == 0:
                print(f"X = {jobs[0]}")
                ...
                print(
                    f"X_hat = {self.tokenizer.decode(torch.argmax(biased_outputs[0], dim=-1), skip_special_tokens=True)}")
            if torch.isnan(loss):
                ipdb.set_trace()
            return loss, mask
        else:
    ...
    def inference(self, jobs, delta_indices, ind_indices, delta_tilde_indices, ind_tilde_indices):
        if self.hp.debug_noiseless == "True":
            ...
            previous_token = self.encoder.embeddings(
                torch.ones(1, 1).type(torch.LongTensor).to(self.decoder.device) * 5)
            ...
            for di in range(max_seq_len - 1):
                decoder_output = self.decoder(
                    ...
                    encoder_hidden_states=encoder_outputs.to(self.decoder.device).float())
                decoder_output_biased = decoder_output.logits #+ atts_tilde_as_bias
                posteriors.append(decoder_output_biased)
                decoder_tok = torch.argmax(decoder_output_biased, dim=-1)
                decoded_tokens.append(decoder_tok)
                previous_token = self.encoder.embeddings(decoder_tok)
    ...
    def training_step(self, batch, batch_nb):
        ...
        if self.hp.denoising_only == "False" and self.epoch_counter > self.hp.min_denoising_epochs:
    ...
    def get_bleu_score(self, test_outputs, test_labels):
        ...
        reshaped_predicted_tokens = test_outputs.view(-1, self.max_len-1)
        reshaped_labels = []
    ...
    def get_att_control_score(self, predicted_tokens, expected_exp, expected_ind):
        ...
        reshaped_predicted_tokens = predicted_tokens.view(-1, self.max_len-1)
