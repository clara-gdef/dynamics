...
import fasttext
import pandas as pd
from models.classes import LSTMAttentionDecoder
...
from utils.models import masked_softmax, embed_and_avg_attributes, sample_y_tilde, compute_bleu_score, \
    handle_fb_preds, get_metrics, get_metrics_at_k, beam_search_decoder, prettify_bleu_score, \
    print_cm
from transformers import CamembertTokenizer, CamembertModel, CamembertForCausalLM


class RewriterDeltaBertIndSub(pl.LightningModule):
    def __init__(self, datadir, emb_size, desc, vocab_size, alpha, beta, model_path, hp):
        ...
        with open(os.path.join(self.datadir, "20_industry_dict.pkl"), 'rb') as f_name:
        ...
        self.pooler = torch.nn.MaxPool1d(self.hp.pooling_window,
                                         padding=0,
    ...
    def forward(self, jobs, delta_indices, ind_indices, delta_tilde_indices, ind_tilde_indices, fwd_type, batch_nb):
        if fwd_type == "DAE":
            ...
            pooled_encoder_outputs = self.pooler(encoder_outputs)
            # labels
            ...
            tmp = torch.bmm(pooled_encoder_outputs, resized_conditioning.transpose(-1, 1))
            attn_weights = masked_softmax(tmp, mask, max_seq_len)
            attn_applied = torch.einsum("beh,bed->bdh", pooled_encoder_outputs, attn_weights)
            decoder_inputs = torch.cat((pooled_encoder_outputs[:, :-1, :], jobs_embedded[:, :-1, :], attn_applied), -1)
            ...
            for di in range(max_seq_len - 1):
                ...
                tmp = torch.bmm(pooled_encoder_outputs, resized_in_token.transpose(-1, 1))
                attn_weights = masked_softmax(tmp, mask, 1)
                assert attn_weights.shape == torch.Size([sample_len, max_seq_len, 1])
                attn_applied = torch.einsum("beh,bed->bh", pooled_encoder_outputs, attn_weights)
                output = torch.cat((previous_token, attn_applied.unsqueeze(1)), -1)
                if self.hp.feed_z == 'True':
                    lstm_input = torch.cat(
                        (pooled_encoder_outputs[:, di, :].unsqueeze(1), previous_token, attn_applied.unsqueeze(1)), -1)
            ...
            pooled_encoder_outputs_2 = self.pooler(encoder_outputs_2)
            ...
            tmp = torch.bmm(pooled_encoder_outputs_2, resized_conditioning.transpose(-1, 1))
            ...
            attn_applied = torch.einsum("beh,bed->beh", pooled_encoder_outputs_2, attn_weights)
            ...
            if self.hp.feed_z == 'True':
                ...
                decoder_inputs = torch.cat((pooled_encoder_outputs_2, embedded_decoded_x_tilde, attn_applied), -1)
    ...
    def inference(self, jobs, delta_indices, ind_indices, delta_tilde_indices, ind_tilde_indices):
        if self.hp.debug_noiseless == "True":
            ...
            prev_hidden_state = (torch.zeros(self.decoder.num_layer, 1, self.decoder.hs).type_as(encoder_outputs),
            ...
            for di in range(max_seq_len - 1):
                ...
                tmp = torch.bmm(pooled_encoder_outputs, resized_in_token.transpose(-1, 1))
                attn_weights = masked_softmax(tmp, mask, 1)
                assert attn_weights.shape == torch.Size([1, max_seq_len, 1])
                attn_applied = torch.einsum("beh,bed->bh", pooled_encoder_outputs, attn_weights)
                output = torch.cat((previous_token, attn_applied.unsqueeze(1)), -1)
                if self.hp.feed_z == 'True':
                    lstm_input = torch.cat(
            ...
            if self.hp.beam_search == "True":
                ...
                if torch.isnan(log_prob).any():
                    ipdb.set_trace()
                selected_seq = torch.zeros(self.hp.test_b_size, self.max_len - 1)
    ...
    def on_test_epoch_start(self):
        ...
        self.classifier_ind = fasttext.load_model(f"/data/gainondefor/dynamics/ft_att_classifier_ind20_10.bin")
    ...
    def test_epoch_end(self, outputs):
        ...
        if self.hp.print_to_csv == "True":
            decoded_sentences = self.tokenizer.batch_decode(stacked_outs.squeeze(1), skip_special_tokens=True)
            exp_as_list = [i.item() for i in initial_exp]
            ind_as_list = [i.item() for i in initial_ind]
            csv_file = print_tilde_to_csv(self.hp.att_type, initial_jobs,
                                          exp_as_list, ind_as_list,
                                          decoded_sentences,
                                          expected_delta, expected_ind,
                                          delta_preds, ind_preds, self.desc,
                                          self.industry_dict, self.delta_dict)
            df = pd.read_csv(csv_file)
            df.to_html(f'html/{self.desc}.html')
        self.metrics = {"Avg ppl": ppl, **bleu, **ind_metrics, **delta_metrics}
        print(self.metrics)
        return self.metrics
    ...
    def get_bleu_score(self, test_outputs, test_labels):
        ...
        for num, predicted_tokens in tqdm(enumerate(reshaped_predicted_tokens),
                                          desc="Building txt files for BLEU score..."):
    ...
    def get_att_control_score(self, predicted_tokens, expected_exp, expected_ind):
        ...
        predicted_inds, predicted_exps = self.get_att_prediction(reshaped_predicted_tokens, mod_type, rev_ind_dict)
        ...
        ind_metrics_at10 = get_metrics_at_k(predicted_inds, ind_labels, len(self.industry_dict), 'ind@5')
    ...
    def get_att_prediction(self, reshaped_predicted_tokens, mod_type, rev_ind_dict):
        ...
        for sequence in tqdm(reshaped_predicted_tokens, desc=f"evaluating attribute control with {mod_type} model..."):
            ...
            predictions_ind = handle_fb_preds(self.classifier_ind.predict(sentence, k=5))
        ...
        return predicted_inds, predicted_exps
    ...
    def get_predictions(self, sentences, ind_indices, delta_indices):
        delta_tilde_indices, ind_tilde_indices = sample_y_tilde(delta_indices, ind_indices, len(self.delta_dict),
                                                                len(self.industry_dict), self.hp.no_tilde)
        decoded_outputs = self.inference(sentences, delta_indices, ind_indices, delta_tilde_indices,
                                         ind_tilde_indices)
        return decoded_outputs, delta_tilde_indices, ind_tilde_indices
